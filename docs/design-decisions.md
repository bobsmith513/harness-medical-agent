...
- **过敏拦截**：输入闸门（词典硬匹配）先于一切 LLM 调用。
  ...
  匹配前完成。
- **对抗样本集**：30 条（`tests/fixtures/adversarial_samples.py`）。
  **范围声明**：这是词典与规则闸门的回归测试集（品牌别名、
  剂型绕过、混合请求、编码变体），**不是** LLM 越狱鲁棒性基准。
  后者需要真实 LLM 端点 + 独立评测流程，本仓库不覆盖。
...
- 总用例数：以 `uv run pytest` 的收集统计为准（README、
  requirements-dev.txt、CI 说明均引用同一数字）；
- `--all-extras` 场景（装齐可选依赖）另有 skip 变化，
  数字不同——文档只引用零依赖口径，避免两套数字打架；
- 覆盖率门禁：CI 中 `pytest --cov` 未达阈值即失败
  （阈值定义见 `pyproject.toml`）。
