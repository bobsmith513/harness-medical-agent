...
**"长会话 Token 降约 81%"的口径**：这是**合成输入下的估算值**
而非实测数字。计算方式——`SessionContext` 只保留最近 3 轮 +
文件指针，溢出旧轮（证据、推理链、摘要）持久化至 VFS 不进
上下文；demo 取 20 轮 × 500 token 的合成输入，压缩后上下文
token 由 `ContextCompactor.estimate_compressed_tokens` 估算，
降幅约 81%。**它不是基准测试结果**：500 token/轮与估算函数
都是 demo 内的假设值，未在真实 tokenizer 与真实 LLM 计费场景下
实测（README demo 总览表标注同口径）。
...
- 总用例数：**634**（2026-09-01 本地实测收集数，`uv run pytest --collect-only`）。
  以 `uv run pytest` 的收集统计为准（README、requirements.txt、
  requirements-dev.txt、CI 说明均引用同一数字）；
- `--all-extras` 场景（装齐可选依赖）为 **630 通过 + 4 跳过**，总收集数
  不变——4 项验证的是"依赖缺失时的降级路径"，装齐后无需重复验证；
  两个数字符合 630 + 4 = 634，文档引用时不要写成"零依赖也是 630"；
- **改测试后必须同步四处**：`README.md`、`docs/design-decisions.md`、
  `requirements.txt`、`requirements-dev.txt`。上一轮只改了 README，
  导致三处停留在 631——数字不一致比数字过期更容易被误读成"没跑过"。
...
评估这个项目的三条线索，全部可本地复跑核对：

1. [development-plan.md](development-plan.md)（每个里程碑的设计取舍）；
2. 本文档（关键实现参数的理由与数据口径）；
3. `tests/`（每项安全与降级语义的断言）。

> **结论一律以产出物与测试为准，不以提交历史为凭据。** 提交历史
> 只说明"按什么顺序做的"；它既不能证明设计正确，也不能证明数字真实。
> 要核对任何一项声明，跑 `uv run pytest` 或对应的 demo 即可。
